...
public final class PlanConstraintsParser
{
    ...
    public static List<PlanConstraint> parse(String querySql)
    {
        if ((querySql.length() - querySql.replace(constraintsMarkerStart, "").length()) / constraintsMarkerStart.length() > 1) {
            throw new PrestoException(SYNTAX_ERROR, "Query cannot have more than one plan hint clause");
        }
        int constraintsStartIndex = querySql.indexOf(constraintsMarkerStart);
        if (constraintsStartIndex == -1) {
            return ImmutableList.of();
        }

        int constraintsEndIndex = querySql.indexOf(constraintsMarkerEnd, constraintsStartIndex);
        String constraintsString = querySql.substring(constraintsStartIndex + constraintsMarkerStart.length(), constraintsEndIndex);

        try {
            return parseInternal(constraintsString.trim().toUpperCase());
        }
        catch (IOException e) {
