        ...
        implements Rule<JoinNode>
{
    ...
    {
        Collection<VariableReferenceExpression> inferredNotNullsVariables;
        switch (joinNode.getType()) {
            case LEFT:
                // NOT NULL can be inferred for the right-side variables
                inferredNotNullsVariables = extractNotNullVariables(joinNode.getCriteria(), joinNode.getFilter(), joinNode.getRight().getOutputVariables(), context.getSession());
                break;
            case RIGHT:
                // NOT NULL can be inferred for the left-side variables
                inferredNotNullsVariables = extractNotNullVariables(joinNode.getCriteria(), joinNode.getFilter(), joinNode.getLeft().getOutputVariables(), context.getSession());
                break;
            case INNER:
                // NOT NULL can be inferred for variables from both sides of the join
                inferredNotNullsVariables = extractNotNullVariables(joinNode.getCriteria(), joinNode.getFilter(), concat(joinNode.getLeft().getOutputVariables().stream(),
        ...
        if (!inferredNotNullsVariables.isEmpty()) {
            Set<VariableReferenceExpression> existingNotNullVars = getExistingNotNullVars(joinNode.getFilter());

            logger.debug("NotNull filters :: Existing : %s, Inferred :%s", existingNotNullVars, inferredNotNullsVariables);

            if (existingNotNullVars.containsAll(inferredNotNullsVariables)) {
                // No new NOT NULL variables were inferred
                return Result.empty();
            }

            RowExpression updatedJoinFilter = and(joinNode.getFilter().orElse(null), buildNotNullRowExpression(inferredNotNullsVariables));
    ...
    @VisibleForTesting
    Set<VariableReferenceExpression> getExistingNotNullVars(Optional<RowExpression> joinFilter)
    {
                ...
                = new DefaultRowExpressionTraversalVisitor<ImmutableSet.Builder<VariableReferenceExpression>>()
        {
            ...
            public Void visitCall(CallExpression call, ImmutableSet.Builder<VariableReferenceExpression> context)
            {
                ...
                if (call.getFunctionHandle().equals(functionAndTypeManager.lookupFunction("not", fromTypes(BOOLEAN))) &&
            ...
            List<VariableReferenceExpression> candidates, Session session)
    {
        ...
        if (joinFilter.isPresent() && getNotNullInferenceStrategy(session) != EQUI_JOIN_ONLY) {
            combinedFilter = and(combinedFilter, joinFilter.get());
        }

        return intersection(copyOf(candidates), inferNotNullVariables(combinedFilter, session));
    }
