...
public class TestPlanConstraintsParser
{
    ...
    public void testSimplePlanConstraints()
    {
        ...
        Optional<String> planConstraintString = Optional.of(constraintsMarkerStart + " join (a b) " + constraintsMarkerEnd);
        ...
        planConstraintString = Optional.of(constraintsMarkerStart + " join (a b) [R] " + constraintsMarkerEnd);
        ...
        planConstraintString = Optional.of(constraintsMarkerStart + " join (a b) [P] " + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 1);
        assertEquals(planConstraints.get(0), expectedJoinConstraintWithPartitionedDistribution);

        planConstraintString = Optional.of(constraintsMarkerStart + " card (foo 1000) " + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 1);
        assertEquals(planConstraints.get(0), expectedCardinalityConstraint);

        planConstraintString = Optional.of(constraintsMarkerStart + " join (a b) card (foo 1000) " + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 2);
        assertEquals(planConstraints.get(0), expectedJoinConstraint);
        assertEquals(planConstraints.get(1), expectedCardinalityConstraint);

        planConstraintString = Optional.of(constraintsMarkerStart + "  card (foo 1000) join (a b) [P] " + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 2);
        assertEquals(planConstraints.get(0), expectedCardinalityConstraint);
        assertEquals(planConstraints.get(1), expectedJoinConstraintWithPartitionedDistribution);

        planConstraintString = Optional.of(constraintsMarkerStart + "  join (a b) [R] card (foo 1000) card (foo 1000) join (a b) " + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
    ...
    {
        Optional<String> planConstraintString = Optional.of(constraintsMarkerStart + " join (a (b c) d) " + constraintsMarkerEnd);
        List<PlanConstraint> planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 1);
        assertEquals(planConstraints.get(0), new JoinConstraint(INNER,
                Optional.empty(),
        ...
        planConstraintString = Optional.of(constraintsMarkerStart + " join (a (b c) [R] d) " + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 1);
        assertEquals(planConstraints.get(0), new JoinConstraint(INNER,
                Optional.empty(),
        ...
        planConstraintString = Optional.of(constraintsMarkerStart + " join ((a (b c) [R]) [P] d e) [P]" + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 1);
        assertEquals(planConstraints.get(0), new JoinConstraint(INNER,
                Optional.of(PARTITIONED),
        ...
        planConstraintString = Optional.of(constraintsMarkerStart + " card ((a b c d) 10)" + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 1);
        ...
        planConstraintString = Optional.of(constraintsMarkerStart + " join ((a (b c) [R]) [P] d e) [P] card ((a ((b c) d e)) 10)" + constraintsMarkerEnd);
        planConstraints = parse(planConstraintString);
        assertEquals(planConstraints.size(), 2);
        assertEquals(planConstraints.get(0), new JoinConstraint(INNER,
                Optional.of(PARTITIONED),
