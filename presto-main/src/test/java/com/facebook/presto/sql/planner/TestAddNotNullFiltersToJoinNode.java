        ...
        extends BasePlanTest
{
    ...
    @Test
    public void testExtractInferredNotNullVariablesVisitorForStandardOperators()
    {
        Session session = testSessionBuilder()
                .setCatalog("local")
                .setSchema("tiny")
                .setSystemProperty(OPTIMIZE_NULLS_IN_JOINS, "true")
                .setSystemProperty(JOINS_NOT_NULL_INFERENCE_STRATEGY, MAP_TO_STANDARD_OPERATOR.name())
                .build();

        ExtractInferredNotNullVariablesVisitor visitor = new ExtractInferredNotNullVariablesVisitor(functionAndTypeManager, session);

        assertInferredNotNullVariableRefsListMatch(visitor, "a + b > 10", buildListOfBigIntVariableRefs("a", "b"));
        assertInferredNotNullVariableRefsListMatch(visitor, "a != 10 - b", buildListOfBigIntVariableRefs("a", "b"));
        assertInferredNotNullVariableRefsListMatch(visitor, "a > b", buildListOfBigIntVariableRefs("a", "b"));
        assertInferredNotNullVariableRefsListMatch(visitor, "a + NULL > b", buildListOfBigIntVariableRefs("a", "b"));

        // We can infer NOT NULL predicates on arguments of an AND expression
        assertInferredNotNullVariableRefsListMatch(visitor, "a > b and c = d", buildListOfBigIntVariableRefs("a", "b", "c", "d"));
        // Cant infer NOT NULL on 'a' but can infer  on 'b > c'
        assertInferredNotNullVariableRefsListMatch(visitor, "a IS NULL and b > c", buildListOfBigIntVariableRefs("b", "c"));

        // arr[3] = 10 translates to EQUAL(SUBSCRIPT(arr, 3), 10). SUBSCRIPT is a standard Operator, so we can infer that 'arr' is NOT NULL
        assertInferredNotNullVariableRefsListMatch(visitor, "arr[3] = 10", ImmutableList.of(variable("arr", new ArrayType(BIGINT))));
        ...
        assertInferredNotNullVariableRefsListMatch(visitor, "c_struct.a = 10", Collections.emptyList());
        // We cannot infer NOT NULL predicates on arguments of an OR expression
        assertInferredNotNullVariableRefsListMatch(visitor, "a > b OR c = d", Collections.emptyList());
        // COALESCE can operate on NULL arguments, so cant infer predicates on its arguments
        assertInferredNotNullVariableRefsListMatch(visitor, "COALESCE(a,b)", Collections.emptyList());
        // IN can operate on NULL arguments, so cant infer predicates on its arguments
        assertInferredNotNullVariableRefsListMatch(visitor, "a IN (b,10,NULL)", Collections.emptyList());
    }

    @Test
    public void testExtractInferredNotNullVariablesVisitorForNonStandardOperators()
    {
        Session session = testSessionBuilder()
                .setCatalog("local")
                .setSchema("tiny")
                .setSystemProperty(OPTIMIZE_NULLS_IN_JOINS, "true")
                .setSystemProperty(JOINS_NOT_NULL_INFERENCE_STRATEGY, USE_FUNCTION_METADATA.name())
                .build();

        ExtractInferredNotNullVariablesVisitor visitor = new ExtractInferredNotNullVariablesVisitor(functionAndTypeManager, session);

        assertInferredNotNullVariableRefsListMatch(visitor, "NOT (b + 10 > c)", buildListOfBigIntVariableRefs("b", "c"));
        assertInferredNotNullVariableRefsListMatch(visitor, "abs(b + c) > 10", buildListOfBigIntVariableRefs("b", "c"));
        assertInferredNotNullVariableRefsListMatch(visitor, "random(b) = ceil(c)", buildListOfBigIntVariableRefs("b", "c"));
    }
    ...
    private void assertInferredNotNullVariableRefsListMatch(ExtractInferredNotNullVariablesVisitor visitor,
