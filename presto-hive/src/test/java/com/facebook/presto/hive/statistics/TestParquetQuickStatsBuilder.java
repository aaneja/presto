...
import static com.facebook.presto.hive.HiveSessionProperties.READ_MASKED_VALUE_ENABLED;
...
public class TestParquetQuickStatsBuilder
{
    ...
    private static void benchmarkS3ReadsDriver()
    {
        ...
        for (int i = 0; i < 5; i++) {
            testParquetQuickStatsBuilder.benchmarkS3Reads(100, true);
        }

        testParquetQuickStatsBuilder.benchmarkS3Reads(1000, false);
    }
    ...
    public void benchmarkS3Reads(int mockedFileCount, boolean isWarmup)
    {
        setUp();
        String bucketPath = "s3a://some-s3-bucket/";
        String partitionPath = "quick_stats";

        ImmutableList<HiveFileInfo> hiveFileInfos = buildHiveFileInfos(bucketPath, partitionPath, mockedFileCount);
        ...
        PartitionQuickStats partitionQuickStats = parquetQuickStatsBuilder.buildQuickStats(SESSION, metastoreMock, new SchemaTableName(TEST_SCHEMA, TEST_TABLE),
                metastoreContext, TEST_SCHEMA, hiveFileInfos);
        sw.stop();
    ...
    public void testStatsBuildTimeIsBoundedUsingFooterFetchTimeout()
            throws Exception
    {
        HiveClientConfig customHiveClientConfig = new HiveClientConfig().setParquetQuickStatsFooterFetchTimeout(new Duration(100, TimeUnit.MILLISECONDS));
